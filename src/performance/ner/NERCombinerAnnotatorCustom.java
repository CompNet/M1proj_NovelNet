...
public class NERCombinerAnnotatorCustom extends SentenceAnnotator  {
  ...
  public NERCombinerAnnotatorCustom(Properties properties) throws IOException {
  ...
  public NERCombinerAnnotatorCustom() throws IOException, ClassNotFoundException {
    this(true);
  }

  public NERCombinerAnnotatorCustom(boolean verbose) throws IOException, ClassNotFoundException {
    this(new NERClassifierCombiner(new Properties()), verbose);
  }

  public NERCombinerAnnotatorCustom(boolean verbose, String... classifiers)
    throws IOException, ClassNotFoundException
  {
    this(new NERClassifierCombiner(classifiers), verbose);
  }

  public NERCombinerAnnotatorCustom(NERClassifierCombiner ner, boolean verbose) {
    this(ner, verbose, 1, 0, Integer.MAX_VALUE);
  }

  public NERCombinerAnnotatorCustom(NERClassifierCombiner ner, boolean verbose, int nThreads, long maxTime) {
    this(ner, verbose, nThreads, maxTime, Integer.MAX_VALUE);
  }

  public NERCombinerAnnotatorCustom(NERClassifierCombiner ner, boolean verbose, int nThreads, long maxTime, int maxSentenceLength) {
   ...
   * 
   * @param annotation
   * @param lToken : un objet TokenNerCustom qui contient la liste des entitées Nommé. par Numéro de phrase dans le texte puis par position du token dans la phrase.
   * @author Schmidt Gaëtan
   * 
   */
  @Override
		    ...
		    if (output == null) {
		      ...
		      for (int i = 0, sz = tokens.size(); i < sz; ++i) {
		        // add the named entity tag to each token
		    	  // remplace neTag par le tag de notre corpus
		       ...
		       // System.out.println("i : "+i);
		      //  System.out.println("Index : "+tokens.get(i).index());
		        //System.out.println("Word : "+tokens.get(i).word());
		      ...
		      if (VERBOSE) {
		        boolean first = true;
		        StringBuilder sb = new StringBuilder("NERCombinerAnnotatorCustom output: [");
  ...
  public void doOneSentence2(Annotation annotation, CoreMap sentence) {
    ...
    if (output == null) {
      ...
      for (int i = 0, sz = tokens.size(); i < sz; ++i) {
        // add the named entity tag to each token
        String neTag = output.get(i).get(CoreAnnotations.NamedEntityTagAnnotation.class);// System.out.println("neTag : "+neTag);
        String normNeTag = output.get(i).get(CoreAnnotations.NormalizedNamedEntityTagAnnotation.class);// System.out.println("normTag : "+normNeTag);
        if (language.equals(LanguageInfo.HumanLanguage.SPANISH)) {
          neTag = spanishToEnglishTag(neTag);
          normNeTag = spanishToEnglishTag(normNeTag);
        }
